...
export class FirecrawlClient {
    ...
    constructor(providedApiKey?: string) {
        ...
        const apiUrl =
            process.env.FIRECRAWL_BASE_URL || process.env.FIRECRAWL_API_URL;

        if (!apiKey && !apiUrl) {
            throw new Error(
                "FIRECRAWL_API_KEY is required - either provide it or set it as an environment variable",
            );
        }

        this.client = new FirecrawlApp({ apiKey: apiKey || "", apiUrl });
    }
    ...
    async search(
        query: string,
        options?: { limit?: number; scrapeOptions?: any },
    ) {
        try {
